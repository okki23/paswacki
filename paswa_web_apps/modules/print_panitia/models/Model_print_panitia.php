...
class Model_print_panitia extends CI_Model {
/*
...
*/
	...
	//ini method untuk menarik seluruh data print_panitia
	public function get_all_print_panitia(){
	...
	public function get_all_print_panitia_detail_json($personnel_id){
	...
	$query = $this->db->query("select a.*,b.name as category, date_format(start_date,'%d-%m-%Y') as tglmulai,date_format(end_date,'%d-%m-%Y') as tglakhir from human_pa_md_emp_training a left join human_cust_personnel_training_category b on b.training_category = a.training_category where a.personnel_id = '$personnel_id' order by start_date desc");
			...
			$list_data = null;
	                ...
	                'opsi'=> '<a href="'.base_url('print_panitia/print_panitia_detail_update').'/'.$row->personnel_id.'/'.$row->start_date.'/'.$row->end_date.'""> <img src="'.base_url('assets/images/edit.png').'" title="Update"> </a><a  onclick="return confirm(Anda Yakin?);"  href="'.base_url('print_panitia/print_panitia_detail_delete').'/'.$row->personnel_id.'/'.$row->start_date.'/'.$row->end_date.'""> <img src="'.base_url('assets/images/delete.png').'" title="Hapus"></a>'
                        //'opsidelete'=> '<a href="'.base_url('print_panitia/print_panitia_detail_delete').'/'.$row->personnel_id.'/'.$row->start_date.'/'.$row->end_date.'""> <img src="'.base_url('assets/images/delete.png').'"> </a>',
	                //'opsiupdate'=> '<a href="'.base_url('print_panitia/print_panitia_detail_update').'/'.$row->personnel_id.'/'.$row->start_date.'/'.$row->end_date.'""> <img src="'.base_url('assets/images/edit.png').'"></a>'
	            );
	        }
	     return $list_data;
	}

	public function print_panitia_detail_delete($personnel_id,$start_date,$end_date){
	...
	public function get_all_pic_print_panitia_json($personnel_id){
	...
	$query = $this->db->query("select *,case when gender = '1' then 'Pria' else 'Wanita' end as genderstatus, date_format(start_date,'%d %M %Y') as tanggalmasuknya from human_pa_md_emp_personal where personnel_id = '$personnel_id' order by personnel_id asc");
	        ...
	        foreach ($query->result() as $row) {
	            $list_data[] = array(
	                ...
	                'tipe_print_panitia'=>$row->tanggalmasuknya,
	                'alasan_print_panitia' => $row->tanggalmasuknya
	            );
	        }
	     return $list_data;
	}
	...
	//ini method untuk menyimpan data print_panitia baru
	public function pro_add_print_panitia($nik,$stor_tanggal_masuk,$nama){
	...
	public function print_panitia_detail($id){
	...
	public function print_panitia_detail_pro_add($data){
	...
	public function print_panitia_detail_update($personnel_id,$start_date,$end_date){ 
	...
	public function print_panitia_update_selected($id){
	...
	public function print_panitia_detail_pro_update($data){
		/*
                personnel_id
                start_date
                end_date
	...
	public function print_panitia_delete($id){
		$query = $this->db->query("delete from human_pa_md_emp_personal where personnel_id = '$id' ");
		return $query;
	}

	public function pro_update_print_panitia($nik,$stor_tanggal_masuk,$nama){
	...
	public function print_panitia_update_pro_selected($data){
